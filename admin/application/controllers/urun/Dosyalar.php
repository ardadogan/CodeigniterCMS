...
class Dosyalar extends MY_Controller {
	...
	{
		$this->header('Dosyalar');

			$data['list'] 	= $this->db->where('urun_id',$urun_id)->get('dosyalar')->result();

			$this->load->view('urun/dosya/list',$data);
	...
	public function yeni($id=false)
	{
		$this->header('Dosyalar');
			...
			$data['id'] = $id;
			$this->load->view('urun/dosya/yeni',$data);
		...
		// Sınıfı Yükle Yükle
			...
			if ( $this->my_upload->uploaded == true  ) {
				...
				if($this->my_upload->processed == true){
					...
					if($this->db->insert('dosyalar',$data)){
	...
	public function duzenle($id=false)
	{
		$this->header('Dosyalar');
			...
			$this->load->view('urun/dosya/duzenle',$data);
	...
	public function sil($id=false)
	{
		...
		if($this->db->delete('dosyalar', array('id' => $id))){
